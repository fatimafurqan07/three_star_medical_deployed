    ...
    <script>
        (function() {
            ...
            function renderLedger(res, start, end) {
                ...
                // ── Customer Profile ──────────────────────────────────────────
                const twinBadge = c.has_twin_vendor
                    ? `<span class="cust-badge" style="background:rgba(255,200,0,.25);border-color:rgba(255,200,0,.4);" title="This customer is also a vendor — purchase transactions are included">🔗 Also a Vendor: ${c.twin_vendor_name || ''}</span>`
                    : '';
            ...
            <div>
                ...
                <div class="cust-meta">
                    <span>📱 ${c.mobile}</span>
                    <span>📍 ${c.address}</span>
                    <span>💳 Opening Bal: Rs ${fmt(c.opening_balance)}</span>
                    ${c.has_twin_vendor ? '<span style="opacity:.8;font-size:.75rem;">⚠️ Net balance = Sales receivable − Purchase payable</span>' : ''}
                </div>
            </div>
        ...
        `;
                ...
                // ── KPI Cards ─────────────────────────────────────────────────
                const netBal = cb;
                const balLabel = netBal > 0.01 ? 'Net Receivable (Dr)' : (netBal < -0.01 ? 'Net Advance (Cr)' : 'Settled ✔');
                const balKlass = netBal > 0.01 ? 'k-red' : 'k-green';
                const drLabel  = c.has_twin_vendor ? 'Total Dr (Sales + Pur.Return)' : 'Total Invoiced (Dr)';
                const crLabel  = c.has_twin_vendor ? 'Total Cr (Receipts + Purchases)' : 'Total Received (Cr)';

                document.getElementById('kpiRow').innerHTML = `
            <div class="kpi-box k-blue">
                <div class="kpi-lbl">Opening Balance</div>
                <div class="kpi-val">Rs ${fmt(Math.abs(ob))}</div>
                <div class="kpi-sub">${ob >= 0 ? 'Receivable' : 'Advance/Cr'}</div>
            </div>
        ...
        `;
            ...
            document.getElementById('btnExportAll')?.addEventListener('click', function() {
                Swal.fire({
                    title: 'Export All Customer Ledgers',
                    html: `
            ...
            function doExportAll(start, end) {
                ...
                fetch(`{{ route('report.customer.ledger.fetch_all') }}?${params}`)
                    .then(r => r.json())
                    .then(res => {
                        document.getElementById('ledLoader').style.display = 'none';
                        ...
                        html2pdf().set({
                            margin: 0,
                            filename: `All_Customer_Ledgers_${start}_to_${end}.pdf`,
            ...
            function renderAllInTemplate(data) {
                ...
                data.all_data.forEach(cust => {
                    const ob = parseFloat(cust.opening_balance);
                    const cb = parseFloat(cust.closing_balance);
                    ...
                    html += `
                        <div class="print-container">
                            ...
                            <div class="p-title">General Ledger</div>
                            ...
                            <table class="p-table">
                                ...
                                <tbody>
                                    <tr>
                                        <td colspan="7"><strong>GL A/C : ${cust.customer.id} - ${cust.customer.name}</strong></td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td class="p-right">${ob < 0 ? fmt(Math.abs(ob)) : '0.00'}</td>
                                        <td class="p-right">${fmt(Math.abs(ob))} ${ob >= 0 ? 'DR' : 'CR'}</td>
                                    </tr>
                                    ${cust.transactions.map(t => `
                                        <tr>
                                            ...
                                            <td class="p-right">${fmt(Math.abs(t.balance))} ${t.balance >= 0 ? 'DR' : 'CR'}</td>
                                        </tr>
                                    `).join('')}
                                </tbody>
                            </table>
                            ...
                            <div class="p-summary-box">
                                <div class="p-summary-title">SUMMARY : ${cust.customer.id} - ${cust.customer.name}</div>
                                <table style="width:100%; font-size:10px; border-collapse: collapse; line-height: 1.1;">
                                    <tr>
                                        ...
                                        <td class="p-right" style="padding: 2px 0;">${fmt(Math.abs(ob))} ${ob >= 0 ? 'DR' : 'CR'}</td>
                                    </tr>
                                    <tr>
                                        <td style="padding: 2px 0;">TOTAL DEBIT AMOUNT</td>
                                        <td></td>
                                        <td class="p-right" style="padding: 2px 0;">${fmt(cust.total_debit)}</td>
                                    </tr>
                                    <tr>
                                        <td style="padding: 2px 0;">TOTAL CREDIT AMOUNT</td>
                                        <td></td>
                                        <td class="p-right" style="padding: 2px 0;">${fmt(cust.total_credit)}</td>
                                    </tr>
                                    <tr>
                                        ...
                                        <td class="p-right" style="padding: 2px 0;"><strong>${fmt(Math.abs(cb))} ${cb >= 0 ? 'DR' : 'CR'}</strong></td>
                                    </tr>
                                </table>
                            </div>
                            <div class="cf"></div>
