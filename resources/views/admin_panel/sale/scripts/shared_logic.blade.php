<script>
  ...
  <tr>
    ...
    <!-- Free Qty -->
    <td class="col-qty">
      <input type="text" class="form-control free-qty text-end" name="free_qty[]" value="0" placeholder="0.0">
    </td>

    <!-- Pack Size -->
    <td class="col-qty">
       <select name="pieces_per_box[]" class="form-select packing-select" style="min-width:110px;"></select>
       <input type="hidden" name="uom_id[]" class="packing-id">
    ...
    function fetchProductPrice($row, productId) {
        ...
        $.get('{{ route('get-price') }}', {
            ...
            if (pRes.size_mode == "by_cartons" || pRes.size_mode == "by_pieces") {
                let p_piece = parseFloat(pRes.sale_price_per_piece) || 0;
                let p_retail = parseFloat(pRes.retail_price) || 0;
                $row.find('.visible-price').val(p_piece > 0 ? p_piece : (p_retail > 0 ? p_retail : 0));
            } else {
                let p_m2 = parseFloat(pRes.price_per_m2) || 0;
                let p_retail = parseFloat(pRes.retail_price) || 0;
                $row.find('.visible-price').val(p_m2 > 0 ? p_m2 : (p_retail > 0 ? p_retail : 0));
            }

            // $row.find('.pack-qty').val(pRes.pieces_per_box || 1);
            var $uomSelect = $row.find('.packing-select');
            $uomSelect.empty();
            ...
            $row.find('.item_uom_factor').val(pRes.pieces_per_box || 1);
            ...
            .done(function(warehouses) {
                ...
                if (validWarehouses.length > 0) {
                    ...
                    validWarehouses.forEach(function(w) {
                        ...
                        const option = 
                            `<option value="${w.warehouse_id}" data-stock="${w.stock}" data-stock-display="${disp}" data-ppb="${w.ppb}" data-size-mode="${w.size_mode}" ${isSel}>${w.warehouse_name} (Stock: ${disp})</option>`;
                        options += option;
                    });
                    $whSelect.html(options);

                    // Re-attach change listener if needed or trigger if preSelected
                    $whSelect.off('change').on('change', function() {
                        const $row = $(this).closest('tr');
                        const $opt = $(this).find('option:selected');
                        const stock = $opt.data('stock') || 0;
                        const stockDisp = $opt.data('stock-display') || stock;
                        
                        $row.find('.warehouse-stock-display').val(stockDisp);
                    });
                ...
                // After loading warehouse, if one is selected, we should try to load batches
                const selectedWh = $whSelect.val();
                if (selectedWh) {
                    loadBatchesForWarehouse($row, productId, selectedWh);
                } else {
                    $row.find('.batch-select').html('<option value="">Auto-FEFO</option>');
                }
            })
            .fail(function(xhr) {
    ...
    function loadBatchesForWarehouse($row, productId, warehouseId) {
        var $batchSelect = $row.find('.batch-select');
        $batchSelect.html('<option value="">Loading...</option>');

        $.get(`/batches/product/${productId}?warehouse_id=${warehouseId}`)
            .done(function(batches) {
                var options = '<option value="">Auto-FEFO (All)</option>';
                if (batches && batches.length > 0) {
                    batches.forEach(function(b) {
                        options += `<option value="${b.id}">${b.label}</option>`;
                    });
                }
            ...
            .fail(function(xhr) {
                ...
                $batchSelect.html('<option value="">Auto-FEFO</option>');
            });
    }
    ...
    function computeRow($row, isManual = false) {
        ...
        const freeInput = $row.find('.free-qty').val() || '0';
        ...
        if (sizeMode === 'by_cartons' || sizeMode === 'by_size') {
            ...
            const fParts = freeInput.toString().split('.');
            const fBoxes = parseInt(fParts[0]) || 0;
            const fLoose = fParts[1] ? parseInt(fParts[1]) : 0;
            freePieces = (fBoxes * packQty) + fLoose;
        } else {
            // Pieces
            ...
            freePieces = toNum(freeInput);
        }

        $row.data('total-free-pieces', freePieces);
    ...
    function updateGrandTotals() {
        ...
        $('#salesTableBody tr').each(function() {
            ...
            tQty += pieces;
            tFreeQty += ($r.data('total-free-pieces') || 0);
            tGross += gross;
            tLineDisc += dam;
            tNet += net;
        });
        ...
        // Display as plain pieces
        $('#tQty').text(tQty > 0 ? tQty + ' Pc' : '—');
        $('#tFreeQty').text(tFreeQty > 0 ? tFreeQty + ' Pc' : '—');
    ...
    function ensureSaved() {
        return new Promise(function(resolve, reject) {
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    $('#btnSave, #btnHeaderPosted, #btnPosted').prop('disabled', false);
                    let errStr = 'Save error';
                    if (xhr.responseJSON) {
                        if (xhr.responseJSON.errors) {
                            errStr = Object.values(xhr.responseJSON.errors).map(e => e.join('\n'))
                                .join('\n');
                        ...
                        try {
                            ...
                            if (json.errors) errStr = Object.values(json.errors).map(e => e.join(
                                '\n')).join('\n');
    ...
    $(document).ready(function() {
        ...
        // Packing change
        $('#salesTableBody').on('change', '.packing-select', function() {
            var $row = $(this).closest('tr');
            var val = $(this).val();

            if (val === 'new_uom') {
                var newName = prompt("Enter New Packing Name (e.g. Box of 24):");
                if (newName) {
                    var factor = prompt("Enter Pieces per " + newName + ":", "10");
                    if (factor && !isNaN(factor) && parseFloat(factor) > 0) {
                        var factorVal = parseFloat(factor);
                        var $opt = $('<option>', {
                            value: 'NEW',
                        ...
                        $row.find('.item_uom_factor').val(factorVal);
                        $row.find('.uom-name-hidden').val(newName);
                        $row.find('.is-new-uom').val('1');
                        $row.find('.packing-id').val('NEW');
                        computeRow($row);
                    } else {
                ...
                $row.find('.packing-id').val($opt.data('id') || val || '');
                
                var nameText = $opt.text().split('(')[0].trim();
                $row.find('.uom-name-hidden').val(nameText);
        ...
        $('#salesTableBody').on('change', '.warehouse', function() {
            ...
            if (productId && warehouseId) {
                loadBatchesForWarehouse($row, productId, warehouseId);
            } else {
                $row.find('.batch-select').html('<option value="">Auto-FEFO</option>');
            }
        });
        ...
        $(document).on('input', '.sales-qty, .pack-qty, .loose-pieces, .discount-value, .visible-price',
            function() {
                ...
                if ($(this).hasClass('visible-price')) {
                    $(this).closest('tr').find('.price-per-piece').val($(this).val());
                }
        ...
        $('#btnPosted, #btnHeaderPosted').off('click').on('click', function() {
            ...
            const creditStatus = $('#creditStatus').val() || 'DO NOT NOTIFY';

            if (creditStatus === 'HOLD ACCOUNT') {
                Swal.fire({
                    icon: 'error',
                    title: 'Account on Hold',
                    text: 'This customer account is on HOLD. No sales allowed.',
            ...
            if (rangeBal > 0) {
                ...
                if (projectedBalance > rangeBal) {
                    const overAmount = projectedBalance - rangeBal;

                    if (creditStatus === 'NOTIFY OVERDUE') {
                        // Hard Block
                        Swal.fire({
                            icon: 'error',
                            title: 'Credit Limit Exceeded',
                            html: `Projected Balance (<strong>${projectedBalance.toLocaleString()}</strong>) exceeds Credit Limit (<strong>${rangeBal.toLocaleString()}</strong>) by <strong>${overAmount.toLocaleString()}</strong>.<br><br>Sale is BLOCKED as per customer settings.`,
                            confirmButtonColor: '#d33'
                        });
                        return;
                    } else {
                        // Soft Warning (DO NOT NOTIFY case)
                        Swal.fire({
                            icon: 'warning',
                            title: 'Credit Limit Warning',
                            html: `Projected Balance (<strong>${projectedBalance.toLocaleString()}</strong>) exceeds Credit Limit (<strong>${rangeBal.toLocaleString()}</strong>) by <strong>${overAmount.toLocaleString()}</strong>.<br><br>Do you still want to proceed with the sale?`,
                        ...
                        return; // Exit to wait for Swal promise
                    }
                }
            }
