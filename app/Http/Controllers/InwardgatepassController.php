...
class InwardgatepassController extends Controller
{
    ...
    {
        $branchId = $this->getBranchId();
        $branches = Branch::when($branchId, fn($q) => $q->where('id', $branchId))->orderBy('name')->get();
        $warehouses = Warehouse::when($branchId, fn($q) => $q->where('branch_id', $branchId))->orderBy('warehouse_name')->get();
        $vendors = Vendor::when($branchId, fn($q) => $q->where('branch_id', $branchId))->orderBy('name')->get();
