...
return new class extends Migration
{
    ...
    Schema::table('hr_employees', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('hr_employees', 'custom_start_time')) {

                $table->time('custom_start_time')->nullable()->after('shift_id');

            }

            if (!Schema::hasColumn('hr_employees', 'custom_end_time')) {
            ...
            if (!Schema::hasColumn('hr_employees', 'face_encoding')) {
                ...
                $table->text('face_encoding')->nullable()->after('custom_end_time');

            } // JSON face descriptor
    ...
    Schema::table('hr_attendances', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('hr_attendances', 'check_in_photo')) {

                $table->string('check_in_photo')->nullable()->after('check_out_time');

            }

            if (!Schema::hasColumn('hr_attendances', 'check_out_photo')) {

                $table->string('check_out_photo')->nullable()->after('check_in_photo');

            }

            if (!Schema::hasColumn('hr_attendances', 'is_late')) {
            ...
            if (!Schema::hasColumn('hr_attendances', 'is_early_leave')) {

                $table->boolean('is_early_leave')->default(false)->after('late_minutes');

            }

            if (!Schema::hasColumn('hr_attendances', 'early_leave_minutes')) {

                $table->integer('early_leave_minutes')->default(0)->after('is_early_leave');

            }

            if (!Schema::hasColumn('hr_attendances', 'total_hours')) {

                $table->decimal('total_hours', 5, 2)->nullable()->after('early_leave_minutes');

            }

            if (!Schema::hasColumn('hr_attendances', 'notes')) {
